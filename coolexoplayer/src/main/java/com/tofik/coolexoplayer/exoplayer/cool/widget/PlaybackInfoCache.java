...
import static com.tofik.coolexoplayer.exoplayer.cool.media.PlaybackInfo.SCRAP;
import static com.tofik.coolexoplayer.exoplayer.cool.widget.Common.ORDER_COMPARATOR_INT;
...
final class PlaybackInfoCache extends RecyclerView.AdapterDataObserver {
  ...
  final void onPlayerAttached(CoolPlayer player) {
  ...
  final void onPlayerDetached(CoolPlayer player) {
    int playerOrder = player.getPlayerOrder();
  ...
  @SuppressWarnings("unused") final void onPlayerRecycled(CoolPlayer player) {
  ...
  @Override
  public void onItemRangeMoved(final int fromPos, final int toPos, int itemCount) {
  ...
  final void savePlaybackInfo(int position, @NonNull PlaybackInfo playbackInfo) {
    CoolUtil.checkNotNull(playbackInfo);
  ...
  @NonNull
  SparseArray<PlaybackInfo> saveStates() {
