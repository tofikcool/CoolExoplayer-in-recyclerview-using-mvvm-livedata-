 ...
 * {@link #getDelayToPlay(CoolPlayer)} returns a positive value will result in a delay in playback
...
public interface PlayerDispatcher {
  ...
  /**
   * Return the number of milliseconds that a call to {@link CoolPlayer#play()} should be delayed.
   ...
   */
  int getDelayToPlay(CoolPlayer player);

  PlayerDispatcher DEFAULT = new PlayerDispatcher() {
    @Override
    public int getDelayToPlay(CoolPlayer player) {
      return DELAY_NONE;
    }
  };
}
