...
public class CoolExoPlayer extends SimpleExoPlayer {

  protected CoolExoPlayer(Context context, RenderersFactory renderersFactory,
  ...
  private CoolPlayer.VolumeChangeListeners listeners;

  public final void addOnVolumeChangeListener(@NonNull CoolPlayer.OnVolumeChangeListener listener) {
    if (this.listeners == null) this.listeners = new CoolPlayer.VolumeChangeListeners();
    this.listeners.add(checkNotNull(listener));
  }

  public final void removeOnVolumeChangeListener(CoolPlayer.OnVolumeChangeListener listener) {
  ...
  public final boolean setVolumeInfo(@NonNull VolumeInfo volumeInfo) {
    ...
    if (changed) {
      ...
      if (listeners != null) {
        for (CoolPlayer.OnVolumeChangeListener listener : this.listeners) {
